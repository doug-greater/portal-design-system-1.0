...
**Dark token model.** The dark theme is a complete inverted-neutral set using **deep tinted blacks** (Vercel/Linear lineage) — **softened in 1.11** (ported from the app): no true-black base, no pure-white text (ink `#F2F2F4`, surface `#1D1D21`, shell `#161619`, borders lifted to `#34343B`/`#45454D`) for gentler long-session contrast. **Elevation in dark is expressed by surface lightening + an inset highlight + a deep shadow** — *not* the light-mode soft drop shadows (see §7). The full block is in `colors_and_type.css` under `html[data-theme="dark"]`.
