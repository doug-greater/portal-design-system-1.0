...
function CapacityControl({ display }) {
  ...
  const unset = val === '';
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', height: 30, borderRadius: 2, overflow: 'hidden', flexShrink: 0,
      ...
      <button onClick={() => setUnit((u) => (u === 'units' ? 'cases' : 'units'))}
        style={{ height: 30, padding: '0 8px', border: 'none', borderLeft: `1px solid ${unset ? 'rgba(185,138,46,.3)' : 'var(--p-border)'}`, background: 'transparent', font: '500 12px Inter', color: 'var(--p-text-2)', cursor: 'pointer' }}>{unit}</button>
    </span>
  );
}
...
function PlacementRow({ seq, name, brand, size, category, plan }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', border: '0.5px solid var(--p-border)', borderRadius: 2, background: '#fff' }}>
      <Icon name="drag_indicator" size={18} color="var(--p-placeholder)" style={{ cursor: 'grab' }} />
      <span style={{ font: '600 11px/1 Geist Mono, monospace', color: 'var(--p-muted)', background: 'var(--p-surface-alt)', borderRadius: 2, width: 22, height: 22, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{seq}</span>
      ...
      <button title="Remove" style={{ width: 28, height: 28, borderRadius: 2, border: 'none', background: 'transparent', color: 'var(--p-placeholder)', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Icon name="close" size={18} /></button>
    </div>
  );
}
...
function SectionCard({ name, count, children, general }) {
  ...
  return (
    <div style={{ border: '0.5px solid var(--p-border)', borderRadius: 2, background: '#fff', boxShadow: 'var(--shadow-card)', marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: 'var(--p-surface-alt)', borderRadius: open ? '10px 10px 0 0' : 10, borderBottom: open ? '0.5px solid var(--p-border)' : 'none' }}>
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, minHeight: 28, marginBottom: 10 }}>
      {editing ? (
        <input autoFocus value={note} onChange={(e) => setNote(e.target.value)} onBlur={() => setEditing(false)}
          ...
          style={{ flex: 1, height: 28, border: '1px solid var(--p-primary)', borderRadius: 2, padding: '0 8px', font: '400 13px Inter', outline: 'none', boxShadow: '0 0 0 3px rgba(24,97,175,.12)' }} />
      ) : (
...
function UnassignedTray({ items }) {
  return (
    <div style={{ border: '1px dashed var(--p-border-strong)', borderRadius: 2, background: 'var(--p-surface-alt)', padding: 10, minHeight: 120 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
        ...
        <span style={{ font: '500 11px/1 Geist Mono, monospace', color: 'var(--p-muted)', background: '#fff', border: '0.5px solid var(--p-border)', borderRadius: 999, padding: '2px 7px' }}>{items.length}</span>
      </div>
      {items.map((p) => (
        <div key={p.name} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 8px', background: '#fff', border: '0.5px solid var(--p-border)', borderRadius: 2, marginBottom: 6, cursor: 'grab' }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 340px', gap: 20, alignItems: 'start', padding: 24, background: 'var(--p-surface-alt)' }}>
      <div>
        <SectionCard name="Cold Shelves" count={2}>
          ...
          <div style={{ marginTop: 8, border: '1px dashed var(--p-border)', borderRadius: 2, minHeight: 52, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, color: 'var(--p-placeholder)', font: '400 13px Inter' }}>
            <Icon name="move_down" size={18} /> Drag products here
          </div>
        </SectionCard>

        <SectionCard name="Back Stock" general>
          <div style={{ border: '1px dashed var(--p-border)', borderRadius: 2, background: 'var(--p-surface-alt)', padding: '28px 16px', textAlign: 'center' }}>
